...
| 🌐 **Your own browser, as a capability** | load a small **browser add-on** into the Chrome you already use, pair it from inside Iron Jarvis, and Jarvis can see your open tabs and name the one you are looking at — no copy-pasting a URL. Ships **off**, and reading is a separate setting from acting. Reading a page's text arrives in v1.236.0 and clicking or typing in v1.237.0; the guarantees they ship with are already decided — page text is untrusted data, so a page cannot give Jarvis orders, passwords are never read, and typed text is redacted in the record |
