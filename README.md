...
| 🌐 **Your own browser, as a capability** | load a small **browser add-on** into the Chrome you already use, pair it from inside Iron Jarvis, and ask about the page in front of you — no copy-pasting a URL. Ships **off**, and looking is a separate setting from acting. Page text is untrusted data, so a page cannot give Jarvis orders; a page that tries is flagged. No field value is ever collected, so a password never leaves your browser. Clicking and typing arrive in v1.237.0, and an external harness driving the same browser in v1.238.0 |
