...
/**
 * Direct REST hookups — the one thing the old Integrations page actually owned
 * (v1.100.0).
 *
 * Integrations and Connections were two sidebar entries for the same job, and
 * the directory half duplicated the sidebar. They are now ONE hub: this section
 * is rendered inside Connections, and /integrations redirects there. Kept as a
 * component rather than inlined so the Connections page doesn't grow past 1,900
 * lines — the same reason chat/page.tsx is hard to work in.
 */
export function RestHookups() {
  ...
  if (offline) return null; // the page already shows one offline banner

  return (
    <Card
      title="Direct REST hookups"
      icon={<Plug size={15} />}
      right={
        <button
          onClick={() => {
