...
export function BatchSuggestCard({
  ...
  instructions?: string;
  /** The same thing, read AT THE PRESS (v1.255.0). The chat page keeps the
   *  composer's text in its own store (v1.250.0, S-05) so the page does not
   *  re-render on every keystroke — which means a `instructions` value passed
   *  down at render is whatever was typed at the last unrelated re-render, not
   *  what is in the box now. Given both, this wins. */
  ...
  async function run() {
    ...
    try {
      const res = await post<{
        ...
        // Read at the press, so the sheet covers what is in the box NOW.
        instructions: (getInstructions?.() ?? instructions ?? "").trim(),
        output: "xlsx",
