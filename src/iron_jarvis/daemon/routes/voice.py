...
def voice_capability(d) -> dict[str, Any]:
    """Whether dictation works right now, and via what — ONE answer for every surface (v1.269.0).

    ``GET /voice/status`` returns exactly this, and the browser add-on's panel reads it
    to offer or grey its microphone. ``backend == "local"`` is the bundled offline
    Vosk model (streaming); anything else is an HTTP transcription backend that takes
    a finished clip.
    """
...
async def transcribe_clip(
    d, *, audio_b64: str, mime: str = "audio/webm", language: str = ""
) -> dict[str, Any]:
    """Transcribe one finished clip through the configured HTTP backend (v1.269.0: shared).

    The body of ``POST /voice/transcribe``, lifted so the browser add-on's panel can
    dictate through the SAME backend and the same model ladder. Raises
    ``HTTPException`` exactly as the route does (424 = no backend / backend refused,
    400 = bad audio, 413 = too large); the route re-raises, the panel turns the
    ``detail`` into one sentence.
    """
    backend = d._voice_backend()
    if backend is None:
        raise HTTPException(
...
def register(app: FastAPI, d) -> None:
    ...
    async def voice_transcribe(body: TranscribeBody) -> dict[str, Any]:
        ...
        return await transcribe_clip(
            d, audio_b64=body.audio_b64, mime=body.mime, language=body.language
        )
